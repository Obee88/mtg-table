...
export function describeEvent(e: RoomEvent, ctx: LogContext): string | null {
  ...
  switch (ev.type) {
    ...
    case 'sideboardingDone':
      return `${ctx.playerName(ev.playerId)} is done sideboarding`;
    case 'handKept':
    ...
    case 'rotisseriePicked':
      return `${ctx.playerName(ev.playerId)} picked ${ev.printingId ? (ctx.printingName?.(ev.printingId) ?? 'a card') : 'a card'} from the table`;
