...
/** Full-height game log column: history backfilled once, then the live event buffer. Scrolls internally only. */
export function LogPanel({ roomId, state, live }: { roomId: string; state: RoomState; live: RoomEvent[] }) {
  ...
  return (
    <aside className={`flex h-full min-h-0 shrink-0 flex-col rounded-lg border border-border bg-surface ${open ? 'w-64' : 'w-8'}`}>
      <button type="button" className="flex shrink-0 items-center justify-between px-2 py-1 text-xs font-medium" onClick={() => setOpen((o) => !o)} title={open ? 'Collapse log' : 'Expand log'}>
        {open ? <span>Log · {lines.length}</span> : <span className="sr-only">Log</span>}
        <span className="text-text-muted">{open ? '›' : '‹'}</span>
      </button>
      {open && (
        <ol className="flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto px-2 pb-2 text-xs">
          {lines.map((l) => (
