    ...
    >
      ...
      {targetColors.length > 0 && (
        <>
          ...
          <span className="pointer-events-none absolute left-1.5 top-1.5 z-10 flex h-6 w-6 items-center justify-center rounded-full bg-black/75 shadow" style={{ color: targetColors[0] }} title={targetColors.length > 1 ? `targeted by  players` : 'targeted'}>
            <svg width="18" height="18" viewBox="0 0 10 10" aria-hidden><circle cx="5" cy="5" r="3" fill="none" stroke="currentColor" strokeWidth="1.2" /><path d="M5 0v2.2M5 7.8V10M0 5h2.2M7.8 5H10" stroke="currentColor" strokeWidth="1.2" /><circle cx="5" cy="5" r="0.9" fill="currentColor" /></svg>
          </span>
        </>
      )}
      {card.noUntap && (
