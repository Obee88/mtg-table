...
export async function roomRoutes(app: FastifyInstance): Promise<void> {
  ...
  /** The caller's own deck as it sits at the table (main = hand + library, by printing), for sideboarding. */
  app.get('/rooms/:id/deck', async (req): Promise<DeckContents> => {
    const { id } = parse(idParam, req.params);
    const state = await app.rooms.get(id);
    if (!state.players[req.user!.id]) throw badRequest('Not in the room');
    return decksFromGame(state)[req.user!.id] ?? { main: [], sideboard: [], commander: [] };
  });
