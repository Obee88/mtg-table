...
const NOT_UNDOABLE = new Set(['roomCreated', 'settingsChanged', 'playerJoined', 'playerLeft', 'seatChanged', 'deckSelected', 'readyChanged', 'roomClosed', 'gameStarted', 'actionUndone', 'mulliganTaken', 'handKept', 'draftStarted', 'draftPicked', 'draftCardReturned', 'draftDeckSubmitted', 'winstonTaken', 'winstonPassed', 'gridTaken', 'winchesterTaken', 'rotisseriePicked', 'resultReported', 'resultProposed', 'resultConfirmed', 'resultRejected', 'sideboardingDone']);
    ...
    await this.db.transaction(async (tx) => {
      ...
      if (picked.length > 0) {
        await tx.insert(schema.draftPicks).values(picked.map((p) => ({
          ...
          phaseType: after.draft!.config.phases[p.phase]?.type ?? null,
        })));
      }
